...
local function GetIconString(msg)
    ...
    if itemID and LootPlusConfig.showLootIcons then
...
local function CreateReadoutFrame(name, labelText, defaultY, configKey)
    ...
    f:SetScript("OnDragStop", function(self) 
        ...
        if addon:IsReady() then 
            ...
            LootPlusConfig[self.configKey].point = p
            LootPlusConfig[self.configKey].x = x
            LootPlusConfig[self.configKey].y = y 
        end 
    end)
...
local function CreateMinimapButton()
    local mmBtn = CreateFrame("Button", "LootPlusMinimapButton", Minimap)
    mmBtn:SetSize(32, 32)
    ...
    mmBtn:SetScript("OnClick", function()
        if ns.UI and LootPlusGUI then
            if LootPlusGUI:IsShown() then LootPlusGUI:Hide() else LootPlusGUI:Show() end
        end
    end)
    
    mmBtn:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_LEFT")
        GameTooltip:AddLine("Loot+ ("..addon.VERSION..")", 1, 1, 1)
    ...
    mmBtn:SetScript("OnDragStart", function(self)
        ...
        self:SetScript("OnUpdate", function()
            ...
            LootPlusConfig.minimap.minimapPos = angle
...
addon.combatFrame = CreateReadoutFrame("LootPlusCombat", "COMBAT & SYSTEM", 150, "combat")
addon.lootFrame = CreateReadoutFrame("LootPlusLoot", "LOOT & MONEY", 50, "loot")
addon.minimapButton = CreateMinimapButton()

function addon:UpdateMinimapPosition()
    if not self:IsReady() then return end
    local pos = LootPlusConfig.minimap.minimapPos or 220
...
function addon:UpdateAllVisuals()
    ...
    if LootPlusConfig.minimap.hide then 
    ...
    local configs = { 
        {f = self.combatFrame, s = LootPlusConfig.combat}, 
        {f = self.lootFrame, s = LootPlusConfig.loot} 
    }
...
function addon:PostTestMessages()
    ...
    local cc = LootPlusConfig.colors
    local cD = cc.delver or {r=1, g=0.7, b=0.2}
    
    self.combatFrame.display:AddMessage(LootPlusConfig.combatEnterText, cc.combatEnter.r, cc.combatEnter.g, cc.combatEnter.b)
    ...
    self.combatFrame.display:AddMessage(LootPlusConfig.combatLeaveText, cc.combatLeave.r, cc.combatLeave.g, cc.combatLeave.b)
    
    local icon = LootPlusConfig.showLootIcons and "|T133644:0|t " or ""
...
addon:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif self:IsReady() then
        local c = LootPlusConfig.colors
        local n = LootPlusConfig.notifications
        
        if event == "PLAYER_REGEN_DISABLED" then
            if n.combatEnter then 
                self.combatFrame.display:AddMessage(LootPlusConfig.combatEnterText, c.combatEnter.r, c.combatEnter.g, c.combatEnter.b)
            end
            return
            
        elseif event == "PLAYER_REGEN_ENABLED" then
            if n.combatLeave then 
                self.combatFrame.display:AddMessage(LootPlusConfig.combatLeaveText, c.combatLeave.r, c.combatLeave.g, c.combatLeave.b)
            end
            return 
        end
        ...
        if isFollowerXP then
            if not LootPlusConfig.showFollowerXP then return end
            if n.xp then
        ...
        elseif event == "CHAT_MSG_MONEY" and n.money then
            if LootPlusConfig.cleanMode and LootPlusConfig.showMoneyIcons then
